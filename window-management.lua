...
for _index,screen in pairs(hs.screen.allScreens()) do
  if screen:frame().w / screen:frame().h > 2 then
    ...
    if screen:frame().w < screen:frame().h then
      ...
      grid.setGrid('9 * 16', screen)
    else
      -- 8 * 4 for normal screen
      grid.setGrid('16 * 9', screen)
    end
  end
end
...
-- move window to left
module.moveLeftWindow = function ()
    local this = windowMeta.new()
    local win = this.window
    local f = win:frame()
    if (f.x - 50 < 0) then
        f.x = 0
        hs.alert.show("Left edge")
    else
        f.x = f.x - 50
    end
    win:setFrame(f)
...
module.moveRightWindow = function ()
    local this = windowMeta.new()
    local win = this.window
    local f = win:frame()
    local fScreen = this.screen:frame();
    if (f.x + f.w + 50 >= fScreen.w) then
        f.x = fScreen.w - f.w
        hs.alert.show("Right edge")
    else
        f.x = f.x + 50
    end
    win:setFrame(f)
...
-- move window Upper
module.moveUpWindow = function ()
    local this = windowMeta.new()
    local win = this.window
    local f = win:frame()
    if (f.y - 50 < 0) then
        f.y = 0
        hs.alert.show("Top edge")
    else
        f.y = f.y - 50
    end
    win:setFrame(f)
...
-- move window to bot
module.moveDownWindow = function ()
    local this = windowMeta.new()
    local win = this.window
    local f = win:frame()
    local fScreen = this.screen:fullFrame()
    if (f.y + f.h + 50 >= fScreen.h) then
        f.y = fScreen.h - f.h
        hs.alert.show("Bottom edge")
    else
        f.y = f.y + 50
    end
    win:setFrame(f)
